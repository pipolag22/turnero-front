...
export default function PuestoPage() {
  ...
  // id robusto para comparar con assignedUserId (id | userId | sub)
  const myUserId = (me as any)?.id ?? (me as any)?.userId ?? (me as any)?.sub ?? null;

  // Tab por defecto según rol
  const [stage, setStage] = useState<Etapa>("BOX");
  useEffect(() => { setStage(me?.role === "PSYCHO_AGENT" ? "PSICO" : "BOX"); }, [me?.role]);

  const queueStage: Etapa = stage === "BOX" ? "RECEPCION" : "PSICO";

  const { enCola, retornoCola, myCalledBox, myAttBox, myCalledPsy, myAttPsy } = useMemo(() => {
    ...
    const myCalledBox = boxTickets.find(t => t.assignedBox === myBox && t.status === "EN_COLA") || null;
    const myAttBox    = boxTickets.find(t => t.assignedBox === myBox && t.status === "EN_ATENCION") || null;

    const psyTickets  = (colas["PSICO"] ?? []);
    const myCalledPsy = psyTickets.find(t => t.assignedUserId === myUserId && t.status === "EN_COLA") || null;
    const myAttPsy    = psyTickets.find(t => t.assignedUserId === myUserId && t.status === "EN_ATENCION") || null;

    return { enCola: colaVisible, retornoCola: finalCola, myCalledBox, myAttBox, myCalledPsy, myAttPsy };
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto" style={{ fontFamily: "system-ui, sans-serif" }}>
      <div className="flex items-center gap-2 mb-4">
        {puedeCambiarTab && (
          <>
            <button className={`px-3 py-2 rounded ${stage === "BOX" ? "bg-blue-600 text-white" : "bg-gray-200"}`} onClick={() => setStage("BOX")}>BOX</button>
            <button className={`px-3 py-2 rounded ${stage === "PSICO" ? "bg-blue-600 text-white" : "bg-gray-200"}`} onClick={() => setStage("PSICO")}>PSICO</button>
          </>
        )}

        {stage === "BOX" ? (
          <div className="ml-auto flex items-center gap-2">
            <button onClick={callRecepcion} className="px-4 py-2 rounded bg-emerald-600 text-white" disabled={busyBox}>Llamar recepción</button>
            <button onClick={callRetorno}   className="px-4 py-2 rounded bg-teal-600 text-white"     disabled={busyBox}>Llamar retorno</button>
          </div>
        ) : (
          <button onClick={callPsy} className="ml-auto px-4 py-2 rounded bg-emerald-600 text-white" disabled={busyPsy}>
            Llamar PSICO
          </button>
        )}
      </div>
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className="col-span-2">
          ...
          <ul className="space-y-2">
            ...
            {enCola.length === 0 && <li className="opacity-60 italic p-3 rounded border">— vacío —</li>}
          </ul>
          ...
          {stage === "BOX" && (
            <>
              ...
              <ul className="space-y-2">
                ...
                {retornoCola.length === 0 && <li className="opacity-60 italic p-3 rounded border">— vacío —</li>}
              </ul>
            </>
          )}
        </div>
        ...
        {/* Panel derecha */}
        <div>
          <h2 className="text-lg font-semibold mb-2">{stage === "BOX" ? `Mi Box ${me?.boxNumber ?? "—"}` : "Mi puesto PSICO"}</h2>
          <div className="p-4 rounded border min-h-[180px] flex flex-col gap-3">
            ...
            {stage === "BOX" && myCalledBox && (
              <>
                ...
                <button onClick={attendBox} className="self-start px-3 py-1 rounded bg-blue-600 text-white">Atender</button>
              </>
            )}
            {stage === "BOX" && myAttBox && (
              <>
                ...
                <button onClick={finishBox} className="self-start px-3 py-1 rounded bg-indigo-600 text-white">Finalizar</button>
              </>
            )}
            ...
            {stage === "PSICO" && myCalledPsy && (
              <>
                ...
                <button onClick={attendPsy} className="self-start px-3 py-1 rounded bg-blue-600 text-white">Atender</button>
              </>
            )}
            {stage === "PSICO" && myAttPsy && (
              <>
                ...
                <button onClick={finishPsy} className="self-start px-3 py-1 rounded bg-indigo-600 text-white">Finalizar (a Retiro)</button>
              </>
            )}
